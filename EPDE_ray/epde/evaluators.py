...
class Custom_Evaluator(Evaluator_template):
    def __init__(self, evaluation_functions : Union[Callable, dict], eval_fun_params_labels : Union[list, tuple, set],
                 use_factors_grids : bool = True):
...
def simple_function_evaluator(factor, structural : bool = False, **kwargs):
    ...
    if factor.params[power_param_idx] == 1:
        if ray.get(global_var.states.parallelize.remote()):
            value_rm = global_var.tensor_cache_actor.call_cache_method.remote('get',
                                                                           factor.cache_label, 
        ...
        label = factor_params_to_str(factor, set_default_power = True, 
                                     power_idx = power_param_idx)
        if ray.get(global_var.states.parallelize.remote()):        
            value_rm = global_var.tensor_cache_actor.call_cache_method.remote('get', label, 
                                                                           structural = structural)
            value = ray.get(value_rm)
        else:
            value = global_var.tensor_cache.get(label, 
                                                structural = structural)            
...
inverse_function_evaluator = Custom_Evaluator(inverse_eval_fun, eval_fun_params_labels = ['dim', 'power'],
