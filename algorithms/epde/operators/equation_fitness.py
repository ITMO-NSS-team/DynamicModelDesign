...
#                torch.nn.Tanh()
    ...
    def apply(self, equation):
        ...
        if ray.get(global_var.states.parallelize.remote()):
            tensor_shape = ray.get(global_var.tensor_cache_actor.call_cache_method.remote('get_arbitrary'))
            var_data = ray.get(global_var.tensor_cache.tensor_cache_actor.call_cache_method.remote('get',
                                                                                           main_var_key))
        else:
            tensor_shape = global_var.tensor_cache.get_arbitrary().shape
            var_data = global_var.tensor_cache.get(main_var_key)
            
        var_modeled = equation.model(self.training_grid).detach().numpy().reshape(tensor_shape)
        rl_error = np.linalg.norm(var_modeled - var_data, ord = 2)
        
        if self.params['verbose']:
            self.plot_data_vs_solution(global_var.tensor_cache.get(main_var_key), var_modeled)
