...
class Poplevel_Right_Part_Selector(Compound_Operator):
    ...
    def apply(self, population, separate_vars = []):
        ...
        '''
        # to_rps = 0
        def operator_func(eq):
            # if eq.right_part_selected:
            self.suboperators['eq_level_rps'].apply(eq, separate_vars) 
        
        if ray.get(global_var.states.parallelize.remote()):
            operator_func = ray.remote(operator_func)
            executable = [operator_func(equation) for equation in population if not equation.right_part_selected]
            ray.get(executable)
        else:            
            for equation in population:
                if not equation.right_part_selected:
                    operator_func(equation)
                # to_rps += 1
...
class Eq_Right_Part_Selector(Compound_Operator):
    ...
    def apply(self, equation, separate_vars):
        ...
        self.suboperators['fitness_calculation'].apply(equation)
        if not np.isclose(equation.fitness_value, max_fitness) and ray.get(global_var.verbose.get.remote()).show_warnings:
...
class Status_respecting_ERPS(Compound_Operator):
    ...
    def apply(self, equation, separate_vars):
		...
		and ray.get(global_var.verbose.get.remote()).show_warnings):
            warnings.warn('Reevaluation of fitness function for equation has obtained different result. Not an error, if ANN DE solver is used.')
        equation.right_part_selected = True    
