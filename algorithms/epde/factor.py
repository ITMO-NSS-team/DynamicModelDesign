...
class Factor(TerminalToken):
    __slots__ = ['_params', '_params_description', 
                 ...
                 '_status', 'equality_ranges', '_evaluator', 'saved']
...
#                raise Exception()
    ...
    def evaluate(self, structural = False):
        ...
        if self.saved[key] and ray.get(global_var.states.parallelize.remote()):
            value_rm = global_var.tensor_cache_actor.call_cache_method.remote('get', self.cache_label,
                                                                          structural = structural)   
            return ray.get(value_rm)
        elif self.saved[key] and not ray.get(global_var.states.parallelize.remote()):
            ...
            if ray.get(global_var.states.parallelize.remote()):
                if key == 'structural' and ray.get(self.status['structural_and_defalut_merged']):
                    temp_rm = global_var.tensor_cache_actor.call_cache_method.remote('use_structural', 
                                                                                     use_base_data = True)
                    ray.get(temp_rm)
                elif key == 'structural' and not ray.get(self.status['structural_and_defalut_merged']):
                    temp_rm = global_var.tensor_cache_actor.call_cache_method.remote('use_structural', 
                                                                           use_base_data = False,
                    ...
                    ray.get(temp_rm)
                     ...
                     saved_state_rm = global_var.tensor_cache_actor.call_cache_method.remote('add', self.cache_label,
                                                                                             value, 
                                                                                             structural = False)
                     self.saved[key] = ray.get(saved_state_rm)
                return value
            else:
    ...
    #     copied_self = self.__class__.__new__(self.__class__)
    #     copied_self.
