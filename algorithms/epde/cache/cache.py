...
def prepare_var_tensor(var_tensor, derivs_tensor, time_axis, boundary, cut_except = []):
    ...
    if isinstance(boundary, int):
        # try:
        result = np.ones((1 + derivs_tensor.shape[-1], ) + 
                 tuple([shape - 2*boundary for shape in var_tensor.shape]))
        # except ValueError:
            # print(var_tensor.shape)
            # result
    elif isinstance(boundary, (tuple, list)):
        result = np.ones((1 + derivs_tensor.shape[-1], ) + 
                 tuple([shape - 2*boundary[dim_idx] for dim_idx, 
...
#        assert not scaled or self.scale_used, 'Trying to add scaled data, while the cache it not allowed to get it'
    ...
    def get_arbitrary(self, normalized : bool = False, structural : bool = False):
        assert not (normalized and structural), 'The added matrix can not be simultaneously normalized and scaled'
        if normalized:
            return next(iter(self.memory_normalized.values()))
        elif structural:
            return next(iter(self.memory_structural.values()))
        else:
            return next(iter(self.memory_default.values()))
