...
class Term(Complex_Structure):
    __slots__ = ['_history', 'structure', 'interelement_operator', 'saved', 'saved_as', 
                 'pool', 'max_factors_in_term', 'occupied_tokens_labels']
    ...
    def Randomize(self, forbidden_factors = None, **kwargs):
        ...
        while True:
            ...
            if forbidden_factors is None:
                break
            elif all([(Check_Unqueness(factor, forbidden_factors) or not factor.status['unique_for_right_part']) 
    ...
    def evaluate(self, structural):
        ...
        if ray.get(global_var.states.parallelize.remote()):
            if self.saved[structural] or (self.cache_label, normalize) in global_var.tensor_cache: #  
                value_rm = global_var.tensor_cache_actor.call_cache_method.remote('get', self.cache_label, 
                                                                               normalized = normalize,
                                                                               saved_as = self.saved_as[normalize]) 
                value = ray.get(value_rm)
                ...
                if np.all([len(factor.params) == 1 for factor in self.structure]):
                    saved_state_rm = global_var.tensor_cache_actor.call_cache_method.remote('add', 
                                                                                            self.cache_label, 
                                                                                            value, 
                                                                                            normalized = normalize) # Место возможных проблем: сохранение/загрузка нормализованных данных
                    self.saved[normalize] = ray.get(saved_state_rm)
    ...
    def Filter_tokens_by_right_part(self, reference_target, equation, equation_position):
        ...
        while True:
            ...
            if accept_term_try == 10 and ray.get(global_var.verbose.get.remote()).show_warnings:
...
class Equation(Complex_Structure): 
    ...
    def solver_form(self):
        if self.solver_form_defined:
            ...
            if ray.get(global_var.states.parallelize.remote()):
                tensor_shape = ray.get(global_var.tensor_cache_actor.call_cache_method('get', '0')).shape
                # print(type(tensor_shape))
                # raise NotImplementedError('Method to be revised')
            else:
                tensor_shape = global_var.grid_cache.get('0').shape
            free_coeff_weight = torch.from_numpy(np.full(shape = tensor_shape, 
                                                              fill_value = self.weights_final[-1]))
            free_coeff_weight = torch.flatten(free_coeff_weight).unsqueeze(1).type(torch.FloatTensor)
            target_weight = torch.from_numpy(np.full(shape = tensor_shape, 
                                                              fill_value = -1))            
    ...
    def max_deriv_orders(self):
        ...
        if ray.get(global_var.states.parallelize.remote()):
            dim = ray.get(global_var.grid_cache_actor.call_cache_method.remote('get', '0')).ndim
        else:
            dim = global_var.grid_cache.get('0').ndim
        max_orders = np.zeros(dim)
    ...
    def boundary_conditions(self, main_var_key = ('u', (1.0,))):
        ...
        if ray.get(global_var.states.parallelize.remote()):
            tensor_shape = ray.get(global_var.grid_cache_actor.call_cache_method.remote('get', '0')).shape
        else:
            tensor_shape = global_var.grid_cache.get('0').shape
        ...
        for ax_idx, ax_ord in enumerate(required_bc_ord):
            ...
            for loc_fraction in hardcoded_bc_relative_locations[ax_ord]:
                ...
                if global_var.states.parallelize.remote():
                    coords_us = [ray.get(global_var.grid_cache_actor.call_cache_method.remote('get', str(idx)))[indexes]
                                 for idx in np.arange(len(tensor_shape))]
                    # coords_unsqueezed = ray.get(coords_rm) 
                    coords = np.squeeze(np.array(coords_us)).T
                    vals_us = ray.get(global_var.tensor_cache_actor.call_cache_method.remote('get', 
                                                                                             main_var_key))
                    vals = np.squeeze(vals_us[indexes]).T
                else:
                    coords = np.squeeze(np.array([global_var.grid_cache.get(str(idx))[indexes] 
        ...
        print('shape of the grid', tensor_shape)
...
def standalone_boundary_conditions(max_deriv_orders, main_var_key = ('u', (1.0,))):
    ...
    hardcoded_bc_relative_locations = {0 : None, 1 : (0,), 2 : (0, 1)}
    if ray.get(global_var.states.parallelize.remote()):
        tensor_rm = ray.get(global_var.grid_cache_actor.call_cache_method.remote('get', '0'))
        tensor_shape = tensor_rm.shape
    else:
        tensor_shape = global_var.grid_cache.get('0').shape
    ...
    def create_equations(self, population_size = 16, sparcity = None, eq_search_iters = None, EA_kwargs = dict()):
        ...
        for eq_idx in range(self.equation_number):
            ...
            if not eq_idx == self.equation_number - 1:
                if ray.get(global_var.states.parallelize.remote()):
                    ray.get(global_var.tensor_cache_actor.call_cache_method('change_variables', 
                                                                            cur_eq_operator_error_abs, 
