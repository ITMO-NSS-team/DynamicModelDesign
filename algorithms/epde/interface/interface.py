...
#        coord_names = ['x' + str(coord_idx) for coord_idx in range(len(self.coord_tensors))]
    ...
    def use_global_cache(self, grids_as_tokens = True, set_grids = True, memory_for_cache = 5, 
        ...
        print(global_var.states.time_axis.remote())
        derivs_stacked = prepare_var_tensor(self.data_tensor, self.derivatives,
                                            time_axis = ray.get(global_var.states.time_axis.remote()), 
                                            boundary = boundary) 
...
class epde_search(object):
    def __init__(self, use_default_strategy : bool = True, eq_search_stop_criterion : Stop_condition = Iteration_limit,
                 ...
                 init_globals : bool = True, example_tensor_shape : Union[tuple, list] = (1000,), 
                 set_grids : bool = True, eq_search_iter : int = 300, use_solver : bool = False, 
                 dimensionality : int = 1, verbose_params : dict = {}, parallelize : bool = False):
        '''
        ...
        '''
        if init_globals:
            global_var.init_globals(parallelize, time_axis, set_grids)        
    ...
    def set_domain_pruning(self, pivotal_tensor_label = None, pruner = None, 
        ...
        if not self.pruner.bds_init:
            if pivotal_tensor_label is None:
                pivotal_tensor_label = ('du/dx'+str(ray.get(global_var.states.time_axis.remote()) + 1), (1.0,))
    ...
    def create_pool(self, data : Union[np.ndarray, list, tuple], time_axis : int = 0, boundary : int = 0, 
                    ...
                    unique_for_right_part : bool = True, memory_for_cache = 5, prune_domain : bool = False,
...
#            print(issubclass(Custom_tokens, Prepared_tokens) or issubclass(Prepared_tokens, Custom_tokens))
    ...
    def terminate(self):
        print('Terminating ray library. For further ')
